...
import 'package:ngopay/douyin/page/error/error_page.dart';
import 'package:ngopay/douyin/utils/app_info.dart';
import 'package:ngopay/douyin/app.dart';
import 'package:ngopay/douyin/service/environment.dart';

void main() {
  //可以从编译参数获取环境变量，用以更改
  const env = String.fromEnvironment('Env', defaultValue: 'prd');
  //初始化环境配置
  environments.init();
...
void _initIOSAndAndroid() {
  //ios、android相关代码
  FlutterBugly.postCatchedException(_initProject, handler: (FlutterErrorDetails details) {
    //继续打印到控制台
...
void _initOther() {
  ...
  runZonedGuarded(_initProject, reportErrorAndLog);
}
